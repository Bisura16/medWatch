...
## Mission Protocol (added 2026-05-18 by autonomous submission mission)

This section supplements the existing rules above. Where the new mission and these rules differ, the conservative reading that satisfies both is chosen.

This repo participates in the MedWatch autonomous submission mission for the dosen hand-over on 25 Mei 2026. Mission control lives in the frontend repo at `~/Documents/FrontendMedWatch/.mission/`. A mirror of mission status lives here at `.mission/STATE-MIRROR.json` and is refreshed at every wave checkpoint.

On session start, read `~/Documents/FrontendMedWatch/.mission/manifest.md` and `~/Documents/FrontendMedWatch/.mission/state.json` and resume at `current_wave`.

Specialised agents live under `.claude/agents/`. All run on `claude-opus-4-7`. Manager session runs at `effortLevel: max`. The 11 agents are: `bug-fixer`, `feature-builder`, `visualization-builder`, `doc-writer`, `test-documenter`, `code-commenter`, `repo-tidier`, `diagram-renderer`, `security-analyst`, `bug-hunter`, `auditor`. Effort tiers: `max` for auditor, bug-fixer, bug-hunter, security-analyst, test-documenter; `xhigh` for the rest.

Per-commit secret scan: `.claude/scripts/secret-scan.sh` blocks `git commit` when staged diff matches forbidden credential patterns. Never write a credential VALUE to any file or commit. Resource NAMES (project `medwatch-polban-2026`, bucket `medwatch-polban-2026-state`, service name, secret name) are OK.

Cloud redeploy of Cloud Run (`gcloud`) and Vercel (`vercel`) is permitted when the mission requires it, using existing env credentials only. Always record pre-change state and verify health post-change; roll back on regression. The account `dudungdotnet@gmail.com` is forbidden in every operation, scan, and document.

No `git push`, no `git push --force`, no `git reset --hard`, no history rewrite. Local commits to `main` only.

The Wave 2 industry-grade README rewrite overrides the prior "README append-only" rule for the dosen submission deliverable: existing README content is restructured into the rewritten README (it is INPUT material, not deleted); teammate-authored sections are preserved within the rewrite. This narrow exception applies only to the submission-package README work.

See `~/Documents/FrontendMedWatch/.mission/plan.md` for the full mission constitution.
